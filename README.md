# CE228724 - HAL I2C MASTER

This application demonstrates how to use I2C driver interface to send and receive bytes or a stream of bytes over the I2C hardware as a master. The on-board LSM9DS1 motion sensor or MAX44009 ambient light sensor or OPT3002 ALS act as the I2C slave. This app uses the Infineon Bluetooth&#174; SoCs and ModusToolbox&#8482;.
Though this document uses CYW20819 the as a reference, the example and instructions are equally applicable for all supported kits listed in the `makefile`
file. However, some of the supported kits need fly-wiring to work correctly.
...
## Hardware Setup
The example kit(cyw920819evb-02) uses the default configuration. Refer to the [kit guide](https://www.infineon.com/cms/en/product/evaluation-boards/cyw920819evb-02/), if required, to ensure the kit is configured correctly.

To make the sample work with CYW989820M2EVB-01 kit, the following hardware setup is required:
 1. P14 (ARD_D8) on J3.10 can be fly-wired to J12.8 to access the ALS interrupt on RSVD_8.
 2. If that setup is desired, use Device Configurator to configure the fly-wired connection.
